...
<head>
    ...
    <title>Create account - Google Classroom</title>
...
<body>
    <div class="auth-card">
        ...
        <?php elseif ($error === 'email_taken'): ?>
            <div class="auth-error">This email is already registered. <a href="login.php" style="color:#1a73e8;">Sign in</a></div>
        ...
        <form action="actions/register_handler.php" method="POST">
            ...
            <p style="text-align:left; font-size:13px; color:#5f6368; margin-bottom:8px; font-family:'Roboto',sans-serif;">I am a</p>
            <div class="auth-role-select">
                <label class="auth-role-option selected" id="roleTeacher">
                    <input type="radio" name="role" value="teacher" checked>
                    <span>Teacher</span>
                </label>
                <label class="auth-role-option" id="roleStudent">
    ...
    <script>
        document.querySelectorAll('.auth-role-option').forEach(opt => {
            opt.addEventListener('click', function() {
                document.querySelectorAll('.auth-role-option').forEach(o => o.classList.remove('selected'));
